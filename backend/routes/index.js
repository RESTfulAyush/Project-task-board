...
api.put("/project/:id/task/:taskId", async (req, res) => {
  try {
    ...
    // Update task inside the project
    const updatedProject = await Project.findOneAndUpdate(
      { _id: req.params.id, "task._id": req.params.taskId },
      {
        $set: {
          ...
          "task.$.priority": value.priority, // ✅ Updating priority correctly
...
api.post("/project/:id/task", async (req, res) => {
  ...
  const taskSchema = joi.object({
    title: joi.string().min(3).max(30).required(),
    description: joi.string().required(),
    priority: joi.string().valid("Low", "Medium", "High").required(), // Added priority validation
  });

  const { error, value } = taskSchema.validate({
    title: req.body.title,
    description: req.body.description,
    priority: req.body.priority, // Capture priority from request body
  });
